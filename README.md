...
### v1.3.0 (当前版本)
- 🚀 **资源优化系统**: 实现动态线程池管理，智能调节并发处理数量
- 📊 **内存监控**: 新增MemoryMonitor类，实时监控内存使用并自动清理
- 📦 **分批处理**: 优化大量文件扫描，采用分批处理避免资源过载
- 🎛️ **性能配置**: 新增performance_config.json，可自定义性能参数
- 🖥️ **UI布局优化**: 重新设计监控面板布局，性能信息移至右侧
- ⚡ **处理速度提升**: 优化文件处理流程，显著提升大批量文件处理效率
- 🛡️ **稳定性增强**: 解决监控大量图片时的卡死问题，提升软件稳定性

### v1.2.0
- 🎨 **JPG质量保护**: 新增JPG格式的质量保护机制，避免重复压缩
- 🌈 **PNG透明度处理**: 优化PNG格式的透明度保护算法
- 🔧 **智能格式处理**: 根据不同图像格式采用最优处理策略
- 📈 **接口优化**: 重构sanitizer_engine.py，提升代码可维护性
