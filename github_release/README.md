...
├── performance_config.json    # 性能配置文件 (v1.3.0新增)
...
### 性能配置 (`performance_config.json`) - v1.3.0新增
```json
{
    "thread_pool": {
        "min_workers": 2,              // 最小线程数
        "max_workers": 8,              // 最大线程数
        "auto_scale": true             // 自动缩放线程池
    },
    "memory_management": {
        "warning_threshold_mb": 500,   // 内存警告阈值(MB)
        "critical_threshold_mb": 800,  // 内存临界阈值(MB)
        "enable_monitoring": true      // 启用内存监控
    },
    "batch_processing": {
        "batch_size": 10,              // 批处理大小
        "processing_delay_ms": 100     // 处理间隔(毫秒)
    }
}
```
